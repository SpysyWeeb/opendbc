...
class LongitudinalController:
  ...
  def calculate_jerk(self, CC: structs.CarControl, CS: CarStateBase, long_control_state: LongCtrlState) -> None:
    ...
    if not self.enabled:
      ...
      # out of the brake-hold ~3x faster off the line (was lumped into the gentle 1.0).
      # 'stopping' stays 1.0 and jerk_lower stays 5.0, so braking/smooth-stops are unaffected.
      self.jerk_upper = 3.0 if long_control_state in (LongCtrlState.starting, LongCtrlState.pid) else 1.0
      self.jerk_lower = 5.0
      return
