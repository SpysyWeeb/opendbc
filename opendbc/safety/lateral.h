...
#include "opendbc/safety/spysypilot/aol.h"
...
bool steer_torque_cmd_checks(int desired_torque, int steer_req, const TorqueSteeringLimits limits) {
  ...
  if (controls_allowed || controls_allowed_lateral || lateral_allowed) {
  ...
  if (!(controls_allowed || controls_allowed_lateral || lateral_allowed) && (desired_torque != 0)) {
    violation = true;
  }
  ...
  if (violation || !(controls_allowed || controls_allowed_lateral || lateral_allowed)) {
    valid_steer_req_count = 0;
    invalid_steer_req_count = 0;
...
bool steer_angle_cmd_checks(int desired_angle, bool steer_control_enabled, const AngleSteeringLimits limits) {
  ...
  if ((controls_allowed || controls_allowed_lateral || lateral_allowed) && steer_control_enabled) {
...
bool steer_curvature_cmd_checks(int desired_curvature, int steer_power, bool steer_control_enabled, const CurvatureSteeringLimits limits) {
  ...
  if (limits.max_steer_power != 0) {
    ...
    violation |= !(controls_allowed || controls_allowed_lateral || lateral_allowed) && (steer_power != 0) && (steer_power >= curvature_state.steer_power_last);
    curvature_state.steer_power_last = steer_power;
  } else {
    // No curvature control allowed when controls are not allowed
    violation |= !(controls_allowed || controls_allowed_lateral || lateral_allowed) && steer_control_enabled;
  }
